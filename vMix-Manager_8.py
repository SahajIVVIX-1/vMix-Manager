...
class VmixParser:
    ...
    def _parse(self):
        ...
        # 2. Initialize all 23 slots. 
        # FIX: Ensure they are marked enabled so they show up in the UI.
        for i in range(23):
            ...
            if i == 0:
                ...
                # If vMix has a name, use it. If not, use 'Empty Cat' but keep it visible.
                name = raw_name if (raw_name and raw_name.strip()) else "Empty Cat"
                is_enabled = enabled_map.get(i, True) # Keep vMix state or default to True
...
class FolderImportWidget(QWidget):
    # Signal now only sends the list of folders/files
    ...
    def __init__(self, parent=None):
        ...
        # Section 1: Modern Queue List
        queue_gb = QGroupBox("📁 Folders to Import (Into selected category)")
        queue_l = QVBoxLayout(queue_gb)
        self.preview = QListWidget()
        self.preview.setStyleSheet(f"background:{C['bg']}; border: 1px solid {C['border']};")
        ...
        self.btn_import.setFixedHeight(55)
        self.btn_import.setStyleSheet(f"background:{C['ok']}; color:white; font-weight:bold; font-size:14px;")
    ...
    def _add_folder_to_queue(self):
        ...
        if folder:
            ...
            files.sort() # Requirement: Ascending sort
            self.queue.append((p.name, files))
            self.preview.addItem(f"Header: [{p.name}] | {len(files)} files found")

    def _clear_queue(self):
        self.queue = []; self.preview.clear()
...
class FactoryResetWidget(QWidget):
    reset_triggered = pyqtSignal(str) # "current", "all", "session"

    def __init__(self, parent=None):
        super().__init__(parent)
        layout = QVBoxLayout(self)
        
        group = QGroupBox("🛠️ Production Management & Factory Reset")
        gl = QVBoxLayout(group)
        gl.setSpacing(10)
        
        desc = QLabel("Use these tools to bulk-delete inputs or reset the entire vMix session.")
        desc.setStyleSheet(f"color: {C['dim']}; margin-bottom: 10px;")
        gl.addWidget(desc)
        ...
        self.btn_wipe_all = QPushButton("🗑️ Wipe ALL Inputs (Globally)")
        self.btn_wipe_all.setStyleSheet(f"background: {C['warn']}; color: black; font-weight: bold;")
        self.btn_wipe_all.clicked.connect(lambda: self.reset_triggered.emit("all"))
        
        self.btn_factory = QPushButton("⚠️ Full Factory Reset (Clear All + Reset Names)")
        self.btn_factory.setStyleSheet(f"background: {C['err']}; color: white; font-weight: bold;")
...
# Add this handler to MainWindow
    ...
    def _on_folder_bulk_import(self, folder_data: list):
        """Processes the queue into the CURRENT selected category from left panel."""
        ...
        for folder_name, file_paths in folder_data:
            # 1. Create the GT Title Header with Font Size 200 and explicit color/style
            # Using the exact field 'Message.Text' as requested
            header_xml = (
