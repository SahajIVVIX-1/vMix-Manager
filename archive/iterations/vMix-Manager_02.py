...
class VmixParser:
    ...
    def swap_categories(self, idx_a: int, idx_b: int):
        ...
        # 1. Swap the names inside the objects (No risk to input titles here)
        name_a = self.categories[idx_a].name
        name_b = self.categories[idx_b].name
        self.categories[idx_a].name = name_b
        self.categories[idx_b].name = name_a

        # 2. Collect all inputs and swap their Category ID
        all_inputs = []
        for cat in self.categories.values():
            all_inputs.extend(cat.inputs)

        for inp in all_inputs:
            if inp.category == idx_a:
                inp.category = 999 # Placeholder
            elif inp.category == idx_b:
                inp.category = idx_a
        ...
        # 3. Physically move the video objects into the other list
    ...
    def save(self, path: str) -> bool:
        try:
            ...
            # 1. Add Inputs (Redistributed by Category ID)
            for cat in sorted(self.categories.values(), key=lambda c: c.index):
                for inp in sorted(cat.inputs, key=lambda x: x.position):
                    # This builds the <Input> tag with the correct Category ID attribute
                    root.append(self._build_el(inp))
            
            # 2. Update the specific Category Label tags
            if self._orig_root is not None:
                for el in self._orig_root:
                    if el.tag in ('Version', 'ZoomManager', 'Input'):
                        continue
                    
                    new_el = copy.deepcopy(el)
                    
                    if new_el.tag == 'CategorySettings':
                        # We iterate through the specific tags defined in your map (Red.Text, etc.)
                        for child in new_el:
                            slot_idx = CAT_ID_LOOKUP.get(child.tag)
                            if slot_idx is not None and slot_idx in self.categories:
                                # TARGETED REPLACE: Only change the text of this specific tag
                                child.text = self.categories[slot_idx].name
