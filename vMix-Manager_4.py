...
import sys, os, json, csv, uuid, copy, re, logging, shutil # <--- ADD shutil
...
class VmixParser:
    ...
    def _parse(self):
        ...
        # 1. Extract existing custom names from XML
        ...
        # 2. Initialize all 23 possible slots
        for i in range(23):
            name = name_map.get(i, "" if i > 0 else "ALL")
    ...
    def _parse_input(self, el: ET.Element) -> VmixInput:
        ...
        return VmixInput(
            ...
            file_path=fp,
            extra_attrs={k:v for k,v in a.items() if k not in {'Key','OriginalTitle','Type','Category','Position'}}
        )
                ...
                for el in self._orig_root:
                    ...
                    if new_el.tag == 'CategorySettings':
                        for child in new_el:
                            ...
                            if idx in self.categories:
                                child.text = self.categories[idx].name # Inject new name
                    root.append(new_el)
